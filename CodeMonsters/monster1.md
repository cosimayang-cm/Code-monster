...
```
                ...
                ▼                 ▼                 ▼
         ┌─────────────┐   ┌─────────────┐   ┌─────────────┐
         │ LaneKeeping │   │ Emergency   │   │ SurroundView│ ⬅️ ✅ 修正
         │             │   │ Braking     │   │ Camera      │
         └──────┬──────┘   └──────┬──────┘   └──────┬──────┘
                │                 │                 │
       ┌────────┼────────┐        │                 │
       ▼        ▼        ▼        ▼                 ▼
┌──────────┐ ┌───────┐ ┌───────────┐         ┌───────────┐
│Navigation│ │Front  │ │  Engine   │         │ RearCamera│
│          │ │Radar  │ │ (運行中)   │         │           │
└────┬─────┘ └───┬───┘ └───────────┘         └─────┬─────┘
     │           │                                 │
     └───────────┴───────────┬─────────────────────┘
...
🔴 已修正：Parking Assist 依賴 SurroundView（不是反過來）
                    ┌─────────────────┐
                    │ Parking Assist  │ ⬅️ ✅ 修正位置
                    └────────┬────────┘
               ┌─────────────┴─────────────┐
               ▼                           ▼
        ┌─────────────┐             ┌─────────────┐
...
🟢 **補充規則（符合真實車輛邏輯）**：
- **引擎依賴中控電腦**：中控電腦關閉時，引擎也必須停止
- 引擎啟動前，必須先開啟中控電腦
- 理由：現代車輛的引擎控制模組、無鑰匙啟動等都依賴電子系統
