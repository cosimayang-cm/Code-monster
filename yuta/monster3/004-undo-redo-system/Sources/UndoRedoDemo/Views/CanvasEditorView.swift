...
struct CanvasEditorView: View {
    ...
    private func createShape(start: CGPoint, end: CGPoint) {
        let modelFillColor = UndoRedoSystem.Color(
        ...
        let modelStrokeColor = UndoRedoSystem.Color(
        ...
        case .circle:
            ...
            viewModel.addCircle(
                at: Point(x: Double(start.x), y: Double(start.y)),
        ...
        case .line:
            viewModel.addLine(
                from: Point(x: Double(start.x), y: Double(start.y)),
...
struct CanvasDrawingView: View {
    ...
    private func drawShape(_ shape: UndoRedoSystem.Shape, in context: GraphicsContext) {
        if let rectangle = shape as? UndoRedoSystem.Rectangle {
            drawRectangle(rectangle, in: context)
        } else if let circle = shape as? UndoRedoSystem.Circle {
            drawCircle(circle, in: context)
        } else if let line = shape as? UndoRedoSystem.Line {
            drawLine(line, in: context)
        }
    }

    /// Draws a rectangle
    private func drawRectangle(_ rectangle: UndoRedoSystem.Rectangle, in context: GraphicsContext) {
        let rect = CGRect(
    ...
    /// Draws a circle
    private func drawCircle(_ circle: UndoRedoSystem.Circle, in context: GraphicsContext) {
        let rect = CGRect(
    ...
    /// Draws a line
    private func drawLine(_ line: UndoRedoSystem.Line, in context: GraphicsContext) {
        var path = Path()
